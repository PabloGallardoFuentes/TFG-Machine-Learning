...
namespace FiltraCSVCompleto
{
    public class FiltroCSV
    {
        ...
        {
            string stringCSV = "IDConductor,CodigoInternoPuntoCarga,LatitudPuntoCarga,LongitudPuntoCarga,CodigoInternoPuntoDescarga,LatitudPuntoDescarga,LongitudPuntoCarga,Label,NumeroParadas,PorcenajePuntoConocido\n";
            int i = 0;
            foreach (DatoML input in inputsML)
            {
                if (input.IDConductor != "")
                {
                    stringCSV += string.Join(",", new string[] { input.IDConductor, 
                        input.CodigoInternoPuntoCarga, input.LatitudPuntoCarga, input.LongitudPuntoCarga,
                        input.CodigoInternoPuntoDescarga, input.LatitudPuntoDescarga, input.LongitudPuntoDescarga,
                        0.ToString(), input.NumeroDeRutasDelConductor.ToString(), input.PorcentajePuntosDeRuta.ToString()}) + "\n";
                }
